...
{
    public static void Show(bool show, bool showBoard)
    {
        ...
        for (int i = 0; i < 7; i++)
        {
            for (int j = 0; j < 8; j++)
            {
                ...
                if (tile.name[1] - '0' > 3 && showBoard) tile.GetComponent<Renderer>().enabled = show;
                if (tile.name[1] - '0' > 3 &&  !showBoard) tile.GetComponent<Renderer>().enabled = false;
                //Bench
